(function(root, factory){
  ...
  function pickJsonLdAuthors(node){
    ...
    function readAuthor(item){
      ...
      if(typeof item === 'object'){
        ...
        else if(item.familyName || item.givenName) authors.push((item.givenName || '') + ' ' + (item.familyName || ''));
      }
    }
    readAuthor(node && (node.author || node.creator));
    return uniqueList(authors.map(function(name){ return asText(name, 256).replace(/\s+/g, ' ').trim(); }).filter(Boolean), 12);
  }
  ...
  function extractAuthors(input){
    ...
    var metaEntries = Array.isArray(source.metaEntries) ? source.metaEntries : [];
    var jsonLdNodes = parseJsonLdBlocks(source.jsonLdTexts);
    var metaAuthors = uniqueList(findMetaContents(metaEntries, ['citation_author', 'dc.creator', 'author']).map(function(name){
      return asText(name, 256).replace(/\s+/g, ' ').trim();
    }), 12);
    if(metaAuthors.length) return detection(metaAuthors.join('; '), 'scholarly_meta', 'strong');
