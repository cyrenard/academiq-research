...
const METADATA_DOI_TIMEOUT_MS = 6500;
const METADATA_SEARCH_TIMEOUT_MS = 5500;
...
function scheduleReactSyncFromLegacy(delay = 120) {
...
function lookupTokens(value: unknown) {
  const stop = new Set(['the', 'and', 'for', 'with', 'from', 'into', 'from', 'using', 'study', 'article', 'bir', 've', 'ile', 'icin', 'olan', 'olarak']);
  return normalizeLookupText(value)
    .split(' ')
    .filter((token) => token.length > 2 && !stop.has(token));
}

function compactLookupText(value: unknown) {
  return normalizeLookupText(value).replace(/\s+/g, '');
}

function ngramSimilarity(a: unknown, b: unknown) {
...
function firstMetadataString(...values: any[]) {
...
function openAlexSourceName(work: any) {
...
function mapSemanticScholarWork(work: any) {
...
function isWeakMetadataValue(value: unknown) {
...
function metadataYear(value: unknown) {
  const match = String(value || '').match(/\b(18|19|20)\d{2}\b/);
  return match ? match[0] : '';
}

function metadataAuthors(value: unknown) {
...
function isPlaceholderTitleForRef(title: unknown, ref: any) {
...
function applyFetchedMetadataToRef(ref: any, fetchedInput: Record<string, any>) {
...
async function fetchOpenAlexMetadataByDoi(doi: string) {
  const cleanDoi = normalizeDoiForMetadata(doi);
  if (!cleanDoi) return null;
...
async function enrichMetadataByDoi(fetchedInput: Record<string, any>, doiInput: unknown) {
...
export function LegacyCompatibilityHost({ onStatus, onImportReferences }: LegacyCompatibilityHostProps) {
  ...
  useEffect(() => {
    return () => {
      if (reactSyncTimer != null) {
        window.clearTimeout(reactSyncTimer);
        reactSyncTimer = null;
      }
    };
  }, []);
