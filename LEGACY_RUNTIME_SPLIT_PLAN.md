...
| 5 | LIBRARY+LABEL no-op render emekliliği | rLib (48 çağrı), rLabelFilter, ctxmenu | ⬜ | BÜYÜK. rLib `getElementById('liblist')`→yok→no-op; React RefSidebar `references={activeWorkspace.lib}` karşılıyor. rLib + label render fn'leri + 48 çağrı yeri + React adapter (legacy-dom-helpers/quality-surface) temizliği. Çok-commit. |
