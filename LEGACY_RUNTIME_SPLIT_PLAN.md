...
| 5b | LABEL render zinciri emekliliği | rLabelFilter, openLabelPickerPanel, showLabelMenu, closeCtxLabelPanel, setLabelFilterPanelOpen, toggleLabelFilterPanel, deleteCustomLabel + activeLabelFilter/labelFilterPanelOpen | ⬜ | Tümü no-op (DOM yok); React RefSidebar karşılıyor. toggleLabelFilterPanel command-palette'ten çağrılıyor (legacy-feature-adapter:199) → o registry satırı da temizlenmeli. ctxmenu çağrı yerleri (openLabelPickerPanel/showLabelMenu) ayıklanmalı. |
